...
def _load_phyjudge_leaderboard() -> dict:
    """Load the PhyJudge-9B auto-eval leaderboard JSON.

    Same lifecycle as the human-eval leaderboard: regenerated by
    `tools/export_phyjudge_leaderboard.py`, copied into staging by
    `tools/build_snapshot.py`. Failing loudly here matches that contract.
    """
    path = REPO_ROOT / "snapshot" / "index" / "phyjudge_leaderboard.json"
    if not path.is_file():
        raise SystemExit(
            f"[build_site] phyjudge_leaderboard.json missing at {path}. "
            f"Run `python3 tools/export_phyjudge_leaderboard.py` then "
            f"`python3 tools/build_snapshot.py`."
        )
    with path.open("r", encoding="utf-8") as fh:
        return json.load(fh)
...
def render(config_path: Path, *, verbose: bool = True) -> None:
    ...
    snapshot_ctx = {
        ...
        "phyjudge_leaderboard": _load_phyjudge_leaderboard(),
    }
